...
for i in range(N):
    ...
    f[i] = mass*ax[i]*vx[i] + mass*ayrar[i]*vy[i] ##f calcula-se para cada ponto

## Altura e velocidante no instante tf

print("Altura após {}s: {:.2f}m.".format(tf,y[-1]))
v = math.sqrt(vy[-1]**2 + vx[-1]**2)
print("Velocidade após {}s: {:.2f}m/s.".format(tf,v))

## Cálculo da energia mecânica

Em = 0.5*mass*(v)**2 + mass*9.8*y[-1]
print("A energia mecãnica após {}s foi {:.1f}J.".format(tf,Em))

## Cálculo do Trabalho
W = dt*((f[0] + f[N-1])*0.5 + np.sum(f[1:N-1]))
print("O trabalho realizado após {}s foi {:.2f}J.".format(tf, W))